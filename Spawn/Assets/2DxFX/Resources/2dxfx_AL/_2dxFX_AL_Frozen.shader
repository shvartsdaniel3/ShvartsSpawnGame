...
#pragma target 3.0
	...
	void surf(Input IN, inout SurfaceOutput o)
	{
		...
		uv = uv*(1 - (_Value2*0.4)) + float2(_Value2*0.2,_Value2*0.2);
		fixed4 t3 = tex2D(_MainTex2, uv) * 2;
		uv = IN.uv_MainTex + float2(-_Value2 - speed,0);
		fixed4 t4 = tex2D(_MainTex2, uv + float2(t2.r,t2.r))*0.5;
		fixed tx = t3.b;
		tx = lerp(0,t3.b*0.015,_Value2);
		fixed4 td = tex2D(_MainTex, IN.uv_MainTex)*IN.color;
		fixed4 t = tex2D(_MainTex, IN.uv_MainTex + float2(tx,tx))*IN.color;
		t2.a = t.a;
